import { Box, Typography, List, ListItem, ListItemText, Divider, RadioGroup, Radio, FormControlLabel, FormControl } from "@mui/material";
        ...
        <List>
          {(viewType === 'multipleChoice' ? questions.multipleChoice : questions.openEnded).map((question, index) => (
            <Box key={question.id}>
              ...
              {index < (viewType === 'multipleChoice' ? questions.multipleChoice : questions.openEnded).length - 1 && <Divider sx={{ my: 2 }} />}
            </Box>
          ))}
        </List>
      </Box>
